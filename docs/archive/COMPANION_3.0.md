# 아카이브 — MEDial 3.0 컴패니언 데모 (이전 방향)

> 이 문서는 **기록**입니다. 현재 활성 제품이 아닙니다.
> 저장소의 현재 방향은 연구용 시뮬레이터이고, 실행 방법은 [README](../../README.md) 와
> [DEVELOPMENT.md](../../DEVELOPMENT.md) 에 있습니다.
>
> 컴패니언 데모 소스는 지우지 않았습니다. `src/App.tsx` 이하와 `server/app/` (simulation 제외)에
> 그대로 있고, 기본 빌드에서만 빠져 있습니다. 다시 보려면:
> `VITE_INCLUDE_COMPANION=1 npm run dev` 후 `#/companion`.
