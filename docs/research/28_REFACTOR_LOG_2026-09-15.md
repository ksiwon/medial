...
## 후속 · 관찰 화면 정리 (같은 날, 연구자 요청 6건 · D102)

1. 지도 헤더의 '새 사례 준비'·'연구자 전체보기'를 없앴다. 새 사례와 MEDial 관점은 재생 줄의 ⚙ 뒤로 갔다.
2. 하루가 05:00부터 22:00까지 흐른다. 서버 덱의 지평을 20:00→22:00으로, `world.day_started`를 00:00→05:00으로 옮겼다(P6·P12의 21:00 귀가가 이제 잘리지 않는다). 재생 막대는 시각이고 22:00 눈금이 '주민 평가'다. 하루가 끝나면 지도 밑에 '22:00 · 하루가 끝났습니다 → 주민 평가 읽기'가 한 줄 나온다. 배속(30·60·120·240×)·한 사건 이동·처음으로는 ⚙ 안이다.
3. 세 열이 456/456/456으로 같다. 셋째 열의 패널이 `flex: 1`이 없어 378px로 좁았다.
4. 헤더의 세 화면은 ① → ② → ③ ↺ 다시 1로.
5. 초록 세로선은 마을 밖(남해읍) 이동의 직선 폴리라인이었다. 요청 경로에서 `offmap` 구간은 그리지 않는다. 지도 저작권 표기는 오른쪽 아래 확대 버튼 밑 9px.
6. 헤더 태그는 '실제 남해군 은점마을 주민 12명'(합성일 때는 '합성 페르소나 · N명').
...
## 후속 · 운전 진술과 조율 화면 (같은 날 · D103)

- 동승 요청이 전원 거절되던 원인은 페르소나의 `drivesSelf=None`이었다. 장부에 `mobility` 연구자 진술(전원 운전, 인터뷰가 말한 사람 제외)을 두고 실행 직전에 덧씌운다. 로컬 장부는 `ledger-eunjeom-v2-draft`.
- 조율 화면: 같은 질문·같은 근거로 후보를 차례로 물은 판단은 한 줄로 합친다("→ P4, P5, 이장 (차례로 물음)"). 질문의 장소 키(`TOWN까지`)는 우리말로.
- '지금 일어난 일'은 최신 사건이 위.
- 확인: pytest 418 · vitest 63 · e2e 6 · 동승 덱 직접 실행(P2→P9, P10→P11 태움).
...
## 후속 · 지도 위의 길과 진행 중 표시 (같은 날 · D106)

연구자가 스크린샷을 짚었다. "허공의 길로 이동하는 버그가 있으니, 반드시 지도 상의 길을
따라서 오도록 해줘." 그리고 "은점마을 지도 상에 있는 길 전부 따올 수 있겠어?"

원인은 등록부의 `roads`였다. 원본 시뮬레이터의 폴리라인은 건물과 건물을 잇는 직선이고,
그 아래 깔린 래스터는 진짜 지도다. 둘을 겹쳐 그려 보니 선이 밭과 바다를 가로질렀고,
읍내행(off-map)은 제 집 마당에서 지도 밖으로 곧장 나갔다.

- `scripts/trace_map_roads.py` — 래스터에서 도로 색(흰 길 · 간선 `#fcd6a4`)을 골라
  골격화하고 폴리라인 277개로 끊는다. 글자 외곽선을 떨어뜨리는 열림(1px)과 글자가
  길 위에 앉아 생긴 틈을 잇는 닫힘(6px)은 "한 덩어리로 이어지고 모든 건물이 길에서
  한 필지 안"이 될 때까지 쓸어 본 값이며, 그 수치를 스크립트 상수로 적어 두었다.
- 장소·집 19곳을 가장 가까운 **선분 위 점**에 붙였다(꼭짓점에 붙이면 집이 제 골목에서
  200m 떨어진다). 가장 먼 곳이 밭 44m.
- `mapRoads` 블록: 폴리라인 · 앵커를 포함한 그래프(미터 가중치) · 원본 거리와의 대조표 ·
  provenance(래스터 해시, 파라미터, 추적 시각). `roads`·`roadGraph`·`namedRouteDistances`는
  건드리지 않았다 — `verify_edge_provenance`의 증명은 연구 기록이라 지울 수 없고, 원본
  거리는 이제 대조군이다(이름난 16개 경로에서 평균 31m, 최대 60m).
- 이동 한 구간은 문 → 길과 만나는 지점 → 길 → 문이고, 진입로도 거리에 포함한다.
  읍내행은 길을 따라 지도 가장자리(TOWNEXIT)까지 간 다음 밖으로 나간다. 시간은 원본의
  고정 30분 그대로다. '마을 밖'은 실제로 지도를 벗어난 뒤부터로 바꿨다 — 전에는 문을
  나서는 순간 지도에서 사라졌다.

같은 날 화면 쪽: 진행 중인 요청마다 색 하나를 주고(끝난 것에는 주지 않는다) 지도의
테두리 · 사건 줄의 띠 · 조율 현황의 줄이 같은 색으로 말하게 했다. 조율 현황은 열린
요청을 위에 두고 끝난 것을 '이전 내역'으로 접으며, 시계를 따라간다(20:42에 위급이
진행 중인데 13:26에 끝난 동승을 설명하고 있었다).

검사: pytest 433(도로 5건 신규) · vitest 63 · e2e 6 · `./run.sh` 기동. 추적한 도로와
하루의 이동 경로를 래스터 위에 겹쳐 사람이 눈으로 확인했다.

고친 김에: `run.sh`의 `http_status`가 연결 실패 시 `000` 위에 `000`을 덧붙여 `000000`이
되었고, 그 값이 "아직 안 떴다"에도 걸리지 않아 vite가 뜨기 전에 스스로 전부 내렸다.
